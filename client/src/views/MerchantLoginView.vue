...
/**
 * 商户登录页 - 与用户登录页布局一致，注册/登录的都是商家账号（role=1）
 */
...
async function handleLogin() {
  ...
  if (role === 0) {
    ElMessage.warning('这是商户登录页，请使用商家或管理员账号')
    userStore.logout()
    return
  }
  ElMessage.success('登录成功')
  if (role === 2) { router.push('/admin') } else { router.push('/seller') }
}
...
async function handleRegister() {
  ...
  if (!valid) return
  await request.post('/auth/register', { username: registerForm.username, password: registerForm.password, role: 1 })
...
async function handlePhoneRegister() {
  ...
  if (!valid) return
  await request.post('/auth/register', { username: phoneRegisterForm.username, password: '123456', phone: phoneRegisterForm.phone, role: 1 })
  ElMessage.success('商家注册成功，请登录')
  isRegister.value = false
      ...
      <div class="login-card">
        ...
        <div class="card-left">
          <div class="left-content">
            ...
            <p class="left-title">商户入驻</p>
            <p class="left-subtitle">开启经营之旅 · 成就商业梦想</p>
          </div>
        </div>
        ...
        <div class="card-right">
          <!-- 商户登录标题 -->
          <h2 class="page-title">商户登录</h2>

          <!-- 登录标签 -->
          <div v-if="!isRegister" class="form-tabs">
            <span class="form-tab" :class="{ active: loginMode === 'password' }" @click="loginMode = 'password'">密码登录</span>
            <span class="form-tab-divider">|</span>
            <span class="form-tab" :class="{ active: loginMode === 'sms' }" @click="loginMode = 'sms'">短信登录</span>
          </div>
          <!-- 注册标签 -->
          <div v-else class="form-tabs">
            <span class="form-tab" :class="{ active: loginMode === 'password' }" @click="loginMode = 'password'">普通注册</span>
            <span class="form-tab-divider">|</span>
            <span class="form-tab" :class="{ active: loginMode === 'sms' }" @click="loginMode = 'sms'">电话注册</span>
          </div>
          ...
          <el-form v-if="loginMode === 'password' && !isRegister" :key="'login-pwd'" ref="loginFormRef" :model="loginForm" :rules="loginRules" label-width="0">
            <el-form-item prop="account">
              <el-input v-model="loginForm.account" placeholder="账号名/手机号" size="large" class="jd-input">
          ...
          <el-form v-if="loginMode === 'sms' && !isRegister" :key="'login-sms'" ref="smsFormRef" :model="smsForm" :rules="smsRules" label-width="0">
            ...
            <el-form-item prop="code">
              <div class="sms-row">
                ...
                <button type="button" class="sms-btn" :class="{ disabled: countdown > 0 }" :disabled="countdown > 0" @click="sendSmsCode">
          ...
          <div class="form-footer">
            <a href="javascript:;" class="footer-link" @click="isRegister = !isRegister; loginMode = 'password'">
              {{ isRegister ? '已有商家账号？去登录' : '立即注册商家' }}
            </a>
            <span class="footer-divider">|</span>
            <router-link to="/" class="footer-link">← 返回用户登录</router-link>
            <span class="footer-divider">|</span>
...
.login-page.dark { background: #1a1a1a; color: #e0e0e0; }
...
.dark .theme-toggle { background: #333; color: #ffd700; border-color: #555; }
.theme-toggle:hover { transform: scale(1.1); }

.login-body { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 40px 30px; }

.login-card {
  width: 100%; max-width: 900px; min-height: 420px; display: flex;
  border-radius: 16px; overflow: hidden;
  box-shadow: 0 4px 30px rgba(0,0,0,0.08); background: #fff; border: 1px solid #f0f0f0;
}
.dark .login-card { background: #2a2a2a; border-color: #444; box-shadow: 0 4px 30px rgba(0,0,0,0.3); }

.card-left {
  width: 380px; background: linear-gradient(135deg, #f8f9fa, #e9ecef);
  display: flex; align-items: center; justify-content: center; flex-shrink: 0;
}
.dark .card-left { background: linear-gradient(135deg, #333, #2a2a2a); }
.left-content { text-align: center; }
.big-logo { width: 80px; height: 80px; object-fit: contain; margin-bottom: 20px; border-radius: 20px; box-shadow: 0 8px 24px rgba(255,103,0,0.3); }
.left-title { font-size: 22px; font-weight: 600; color: #333; margin-bottom: 8px; }
.dark .left-title { color: #fff; }
.left-subtitle { font-size: 14px; color: #999; }

.card-right { flex: 1; padding: 40px 50px; display: flex; flex-direction: column; justify-content: center; }

.page-title { font-size: 22px; font-weight: 600; color: #333; margin-bottom: 20px; }
.dark .page-title { color: #fff; }

.form-tabs { display: flex; align-items: center; gap: 12px; margin-bottom: 30px; }
.form-tab { font-size: 18px; cursor: pointer; color: #999; transition: color 0.3s; }
.form-tab.active { color: #ff6700; font-weight: 600; }
...
.sms-input { flex: 1; }
.sms-btn {
  height: 40px; padding: 0 16px; border: 1px solid #ff6700;
  background: #fff; color: #ff6700; border-radius: 8px;
  cursor: pointer; font-size: 13px; white-space: nowrap;
  transition: all 0.3s; flex-shrink: 0;
}
.sms-btn:hover:not(.disabled) { background: #ff6700; color: #fff; }
.sms-btn.disabled { border-color: #ccc; color: #999; cursor: not-allowed; }

.jd-input :deep(.el-input__wrapper) { border-radius: 8px; box-shadow: 0 0 0 1px #e0e0e0 inset; background: #f8f9fa; transition: all 0.3s; }
.dark .jd-input :deep(.el-input__wrapper) { background: #3a3a3a; box-shadow: 0 0 0 1px #555 inset; }
.jd-input :deep(.el-input__wrapper:hover) { box-shadow: 0 0 0 1px #ccc inset; }
.jd-input :deep(.el-input__wrapper.is-focus) { box-shadow: 0 0 0 2px #ff6700 inset; }
...
.login-btn {
  ...
  background: linear-gradient(135deg, #ff6700, #ff8533); color: #fff;
  font-size: 16px; font-weight: 500; letter-spacing: 6px; cursor: pointer; transition: all 0.3s;
}
.login-btn:hover { filter: brightness(1.05); box-shadow: 0 4px 16px rgba(255,103,0,0.4); transform: translateY(-1px); }
.login-btn:active { transform: translateY(0); }
.register-btn { background: linear-gradient(135deg, #e1251b, #ff4e3a); }

.form-footer { margin-top: 20px; display: flex; align-items: center; gap: 12px; font-size: 13px; }
.footer-link { color: #999; cursor: pointer; transition: color 0.2s; text-decoration: none; }
.footer-link:hover { color: #ff6700; }
.footer-divider { color: #ddd; }
.dark .footer-divider { color: #555; }

.login-footer { padding: 24px 30px; text-align: center; border-top: 1px solid #f0f0f0; }
.dark .login-footer { border-top-color: #333; }
.footer-links { display: flex; align-items: center; justify-content: center; gap: 16px; }
.footer-links a { font-size: 12px; color: #999; transition: color 0.2s; text-decoration: none; }
