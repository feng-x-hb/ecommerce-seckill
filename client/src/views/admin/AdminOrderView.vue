...
async function fetchData() {
  loading.value = true
  try {
    let url = `/admin/order/list?page=${page.value}&size=${size.value}`
...
async function handleShip(orderNo: string) {
  ...
  await request.put(`/admin/order/${orderNo}/ship`)
  ElMessage.success('已标记为发货')
  fetchData()
}

async function handleReject(orderNo: string) {
  await ElMessageBox.confirm(`确定驳回订单 ${orderNo}？将取消该订单。`, '驳回确认', { type: 'warning' })
  await request.put(`/admin/order/${orderNo}/reject`)
...
<template>
  <div class="admin-page">
    <div class="search-bar">
      <el-input v-model="keyword" placeholder="搜索订单号" clearable style="width: 240px" @keyup.enter="handleSearch" />
    ...
    <el-table :data="tableData" v-loading="loading" stripe border>
      ...
      <el-table-column label="操作" width="220" fixed="right">
        <template #default="{ row }">
          ...
          <el-button v-if="row.status === 1" type="danger" size="small" link @click="handleReject(row.orderNo)">驳回</el-button>
        </template>
      </el-table-column>
    </el-table>
      ...
      <template v-if="detail">
        <el-descriptions :column="2" border size="small">
          ...
          <el-descriptions-item label="状态">
            <el-tag :type="(statusMap[detail.status]?.type as any) || 'info'" size="small">
              {{ statusMap[detail.status]?.label }}
            </el-tag>
          </el-descriptions-item>
          <el-descriptions-item label="买家">{{ detail.buyerName }} ({{ detail.buyerPhone }})</el-descriptions-item>
          ...
          <el-descriptions-item label="下单时间">{{ detail.createdAt?.replace('T', ' ').slice(0, 19) }}</el-descriptions-item>
          <el-descriptions-item label="支付时间">{{ detail.payTime?.replace('T', ' ').slice(0, 19) || '未支付' }}</el-descriptions-item>
        </el-descriptions>
